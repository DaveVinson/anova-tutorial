...
### First we need to determine if distribution abides by assumptions 
...
There are a few ways to test for normality (no Skew or Kurtosis). The Shapiro test is the most conservative test.
```{r shap}
shapiro.test(results$newx) #super conservative
```

The q-q plot (or quantile by quantile plot) shows whether the first half of the data is different than the second half (e.g., did it come from the same population?)
```{r qq}
...
Trimming can be applied when your data appear to be centered, but not normal (e.g., kurtosis exists) due to outliers on both sides of the distribution. 
...
```
Looks good! So now, lets look to see if we've found a signficiant effect in our anova
...
We do! Let's run a Tukey SD analysis for simple effects 
```{r Simple effects}
